	...
	namespace Assassination
	{
		void Enable(bool enabled)
		{
			...
			if (enabled) {
				...
				for (auto i = 0; i < 50; i++)
					spartan->Animations[i].FrameCount = 0;
			}
		}

		void Tick()
		{
			if (!Modules::ModuleServer::Instance().VarServerAssassinationEnabledClient->ValueInt)
				Assassination::Enable(false);
			else
				Assassination::Enable(true);
		}
	}
}